...
\newcommand{\nyi}[1]{\underline{#1} (not yet implemented)}
...
(\verb+`+) is an ordinary identifier.  That is, surrounding an
...
\begin{tabular}{|c|l|}
  ...
  % \hline \verb+\u+\emph{hhhh} & the unicode character with code \emph{hhhh} (where \emph{h} is a hex digit)  \\
  % \hline \verb+\U+\emph{hhhhhhh} & the unicode character with code \emph{hhhhhhhh} (where \emph{h} is a hex digit)  \\
  \hline \verb+\v+ & vertical tab (ascii code \verb+0x0B+) \\
  \hline \verb+\x+\emph{hh} & the character with code \emph{hh} (where \emph{h} is a hex digit) \\
