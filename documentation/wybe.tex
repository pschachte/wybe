...
one or more expressions at precedence -1 or above separated by
vertical bars, or one or more expressions at precedence 0 or above
separated by commas, will be read a set.
For example, \verb+{a,b,c}+ is interpreted as
\verb+`{,}`(c,`{,}`(b,`{,}`(a,{})))+.
Sets with elements of the form \emph{a}\texttt{::}\emph{x} are
handled specially.
For example,
\verb+{a::x,b::y,c::z}+ is interpreted as
\verb+`{:,}`(c,z,`{:,}`(b,y,`{:,}`(a,x,{})))+.
