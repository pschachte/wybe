...
numberPStmtVars vars placed pos = do
    liftIO $ putStrLn $ "Numbering     " ++ show (content placed)
    liftIO $ putStrLn $ "  with vars = " ++ show vars
    (stmt,vars') <- 
        numberStmtVars vars (content placed) (betterPlace pos placed)
    liftIO $ putStrLn $ "  result    = " ++ show stmt
    liftIO $ putStrLn $ "  new vars  = " ++ show vars'
...
addVarDef vars v (pos:_) = Map.insertWith (+) v 1 vars
...
numberPExpsVars vars exps pos =
    foldM (numberOneExp vars pos) ([],noVarDefs) exps
