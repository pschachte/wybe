%  Emacs    : -*- latex -*-
%  File     : design.tex
%  RCS      : $Id: design.tex,v 1.4 2004/04/01 23:57:17 schachte Exp $
...
handled by the parser as identifiers, and coerced to the right type by
the semantic analysis (handled by the compiler, rather than the
lexer)?   Strings would seem like an even bigger problem.

The current working name for the language is \frege, for
\textbf{F}unctions, \textbf{RE}lations, and \textbf{GE}nerics.
...
object files, is \emph{not} necessary.  The real goal is to be able to
quickly begin testing code after a small change.  One would usually
prefer to use a tool like \verb@make@ to rebuild an executable than to
...
Therefor, There would have to be a mechanism for declaring mutual
...
large enough to
hold the address of an address, and a addressible
...
This needs to be
generalized to allow different approaches for addressing characters
...
since \verb@true@ used as predicate could be defined to succeed and \verb@false@
...
\verb@append@/3 would specify that on completion, the backbone of the first
...
instantiation for the \verb@plus@/3 predicate (and the \verb@+@/2 function) might
...
\verb@append(A,B,C)@ must say that on success, the spine of \verb@A@ is fully bound,
and the spine of \verb@B@ is iff the spine of \verb@C@ is, and that the elements of
\verb@C@ are fully bound iff those of both \verb@A@ and \verb@B@ are.
...
\verb@definite@, meaning ground and lacking any alternatives, which is more
specific than \verb@ground@.  A predicate's determinism could then be
specified as part of its mode.  The determinism of \verb@append(A,B,C)@, eg,
...
want to say that \verb@append(A,B,C)@ may fail if both \verb@B@ and \verb@C@ are bound, or
if both \verb@A@ and \verb@C@ are bound.
...
\verb@append(A,B,C)@ will never leave a choicepoint if the spine of \verb@A@ is
fully bound.  Whether \verb@B@ is bound or not is immaterial to choicepoints.
...
\verb@p@/2.  This approach also is not attached to modes; a predicate would
...
mode.  Eg,  \verb@reverse@/3 could have a default of \verb@[]@ for the third
argument; then \verb@reverse(AB, BA)@ would mean \verb@reverse(AB, BA, [])@,
...
smoothly, but it wouldn't.  A higher order term like \verb@reverse(AB)@ is
...
initializes a cursor, and one that takes a cursor as input and
...
independetly.  One example of this would be to turn a conjunction of
...
\emph{impure} determinisitic, but not to \emph{pure} determinisitic
code.
...
allows one to to \emph{read} such terms without worrying about
...
p(X, Y), so committment to an earlier solution of an or else operator
