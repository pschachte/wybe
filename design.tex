%  Emacs    : -*- latex -*-
%  File     : design.tex
%  RCS      : $Id: design.tex,v 1.8 2007/04/16 13:05:41 schachte Exp $
...
operation may belong in multiple modules.  This, presumably, is why object
oriented languages single out one argument to ``receive'' a message, with any
other arguments just coming along for the ride.  This asymmetry is rather
unpleasant, doesn't fit well with higher-order programming, and should be
avoided if possible.  Naturally, each operation must belong only to the
single module it is defined in.

\Frege supports two kinds of modules.
...
\Frege supports three approaches to module composition:  layering, extension,
