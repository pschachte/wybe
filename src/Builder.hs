...
compileModSCC mspecs = do
    ...
    ----------------------------------
    -- FLATTENING
    fixpointProcessSCC (handleModImports BeforeCompleteNormalisation) mspecs
    -- Must be done before flattening so flattening recognises use of resources 
...
handleModImports :: ImportPhase -> [ModSpec] -> ModSpec
                 -> Compiler (Bool,[(String,OptPos)])
handleModImports phase _ thisMod = do
    ...
    mapM_ (uncurry (doImport phase)) $ Map.toList imports
    maybe (return ()) (importFromSupermodule phase) nestedIn
    when (phase == AfterCompleteNormalisation) publishTraitImpls
    kTypes'     <- getModuleImplementationField modKnownTypes
    kResources' <- getModuleImplementationField modKnownResources
    kProcs'     <- getModuleImplementationField modKnownProcs
    kTraitImpls' <- getModuleImplementationField modKnownTraitImpls
    ...
    return (kTypes/=kTypes' || kResources/=kResources' ||
            kProcs/=kProcs' || kTraitImpls/=kTraitImpls' ||
            iface/=iface',[])
...
buildExecutable orderedSCCs targetMod target = do
    ...
    if "" `elem` procs || "<0>" `elem` procs
        then do
            ...
            importFromSupermodule AfterCompleteNormalisation targetMod
