...
flattenStmt' (And tsts) pos detism = do
    ...
    emit pos $ And tsts'
flattenStmt' (Or [] _ _) pos detism = -- never generated by parser
    emit pos Fail
flattenStmt' (Or [single] vars res) pos detism = -- never generated by parser
    placedApply flattenStmt' single detism
-- transform disjunction into conditional
flattenStmt' (Or (tst1:tst2:tsts) vars res) pos detism = do
    let stmt = transformDisj tst1 tst2 tsts vars res
    flattenStmt' stmt pos detism
...
-- | Transform a disjunction into a conditional stmt.

transformDisj :: Placed Stmt -> Placed Stmt -> [Placed Stmt] -> Maybe VarDict
              -> Maybe (Set ResourceSpec) -> Stmt
transformDisj disj1 disj2 [] vars res =
    Cond disj1 [Unplaced Nop] [disj2] Nothing vars res
transformDisj disj1 disj2 (disj3:disjs) vars res =
    Cond disj1 [Unplaced Nop]
        [Unplaced $ transformDisj disj2 disj3 disjs vars res] Nothing vars res
