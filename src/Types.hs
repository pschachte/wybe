...
instance Show CallInfo where
    ...
    show (HigherInfo fn mods) 
        | mods == defaultProcModifiers = "higher " ++ show fn
        | otherwise = "higher " ++ show fn
                                ++ " with modifiers " ++ showProcModifiers mods
    ...
    case stmt of
        ProcCall (First m name procId) d resful args -> do
            ...
            then let var = varGet name
                 in return $ StmtTypings pstmt $ [TestInfo var | couldBeTest]
                                              ++ concat [higherInfos var varTy resful | couldBeHigher]
            else do
        ...
        ProcCall (Higher fn) _ resful _ ->
            return $ StmtTypings pstmt $ higherInfos (content fn) AnyType resful
        _ ->
...
firstInfo def proc = do
    let proto = procProto def
        ...
        needsResBang = not (Set.null inResources && Set.null outResources)
...
higherInfos :: Exp -> TypeSpec -> Bool -> [CallInfo]
higherInfos fn@(Typed _ (HigherOrderType mods _) _) _ _ = [HigherInfo fn mods]
higherInfos fn          (HigherOrderType mods _)      _ = [HigherInfo fn mods]
higherInfos exp _ bang =
    [HigherInfo exp defaultProcModifiers{modifierResourceful=resful,
                                         modifierImpurity=impurity}
    | resful <- False : [True | bang]
    , impurity <- Pure : [Semipure | bang]]
...
matchTypes caller callee pos _ callTypes callFlows
        calleeInfo@(HigherInfo fn mods) = do
    ...
    typing <-
        case fnTy of
            HigherOrderType mods' tfs | mods == mods' -> do
