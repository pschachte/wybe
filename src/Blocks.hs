...
_translateProcImpl modProtos proto isClosure body startCount = do
    ...
    body' <- tryMakeTailCall body
    logBlocks $ "modified body: " ++ show body'
    -- Codegen
...
tryMakeTailCall :: ProcBody -> Compiler ProcBody
tryMakeTailCall = mapProcLeaves
    (\prevBlocks lastBlock -> do
        case partitionLastCall lastBlock of
            (Just (beforeCall, call), afterLastCall@(_:_)) -> do
                let lastCall = content call
                logBlocks "trying to make the last call a tail call"
                logBlocks $ "lastCall: " ++ show beforeCall ++ "\ncall:" ++ show lastCall ++ "\nafterLastCall: " ++ show afterLastCall
                ...
                logBlocks $ "failed: " ++ show (reverse remainingAfterLastCall) ++ "\nsucceeded:" ++ show (reverse movedBeforeLastCall)
                ...
                case foldrM (analyzePrimAfterLastCall (content call)) [] (reverse (List.map content remainingAfterLastCall)) of
                    Right mutateChains -> do
                        ...
                        if all (allUnique . List.map offsetBytes) mutateChains
                        then do
                            logBlocks "remaining prims meet requirements for last call -> tail call transform"
                            logBlocks $ show mutateChains
                            return $ beforeCall ++ reverse movedBeforeLastCall ++ [call] ++ reverse remainingAfterLastCall
                        else do
                            logBlocks "mutate instructions alias! aborting"
                            return lastBlock
                    Left error -> do logBlocks $ "remaining prims didn't satisfy constraints: " ++ error
                                     return lastBlock
            _ -> do
                logBlocks "nothing to optimize: either no last call, or last call is already in tail position"
                return lastBlock
    )
...
analyzePrimAfterLastCall :: LastCall -> Prim -> MutateChains -> Either String MutateChains
analyzePrimAfterLastCall lastCall prim state = case prim of
    PrimForeign "lpvm" "mutate" _ mutateInstr@[
      ...
      | valueName `elem` varsInPrim FlowOut lastCall -> if not final
          ...
          else tryAddToMutateChain lastCall state MutateInstr { inputName = inputName, outputName = outputName, offsetBytes = offset, sizeBytes = size, valueName = valueName } state
...
tryAddToMutateChain :: LastCall -> MutateChains -> MutateInstr -> MutateChains -> Either String MutateChains
...
_partitionLastCall (x:xs) =
    case content x of
        ...
        PrimHigher {} -> (Just (reverse xs, x), [])
        PrimForeign {} -> let (lastCall, afterLastCall) = _partitionLastCall xs
